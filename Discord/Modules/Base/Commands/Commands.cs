...
using DiscordBot.Discord.Modules.Base.Embeds;
using DiscordBot.Discord.Modules.Event;
using DiscordBot.Discord.Modules.Logging.Embeds.Member;

namespace DiscordBot.Discord.Modules.Base.Commands
{
    public class Commands : InteractionModuleBase<ShardedInteractionContext>
    {
        ...
        {
            var messageList = await Context.Channel.GetMessagesAsync(amount).FlattenAsync();

            await (Context.Channel as SocketTextChannel).DeleteMessagesAsync(messageList);

            await RespondAsync("Deleted " + amount + " message(s)", ephemeral: true);
        }
        ...
        public async Task SetupBot()
        {
            try
            {
                JoinedGuild.AddGuild(Context.Guild, Bot.Services);
                JoinedGuild.DownloadMembers(Context.Guild.Users, Context.Guild.Id, Bot.Services);
                JoinedGuild.SetGuildOwner(Context.Guild.OwnerId, Context.Guild.Id, Bot.Services);
                JoinedGuild.GenerateDefaultViolation(Context.Guild, Bot.Services);
